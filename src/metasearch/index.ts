...
async function main() {
  ...
  const parsedQueries = entries.map(({ parse }) => parse(url))
  const active = parsedQueries.findIndex(Boolean)

  if (active === -1) return
  debug?.('Active search engine index:', entries[active]?.title, active)
  ...
  const filtered = entries.filter((_, i) => i !== active)

  const activeQuery = parsedQueries[active]
  if (!activeQuery) return

  const q = encodeURIComponent(activeQuery)
  for (const { format, render } of filtered) container.append(render(format(q)))
...
function getActiveAnchor(): HTMLAnchorElement {
...
function createItem(_engine: Engine, mobile = false) {
  ...
  return {
    ...engine,
    shortcut,
    parse: (current = new URL(window.location.href)): string | undefined | false | null => {
      let parsedQuery: string | undefined | false | null

      if (current.host !== host) return parsedQuery

      if (typeof parse === 'function') parsedQuery = parse(current)
      else if (parse instanceof RegExp) parsedQuery = current.href.match(parse)?.[0]
      else {
        ...
        if (site && q?.includes('site:')) {
          ...
          parsedQuery = siteName === site ? siteQuery : undefined
        } else parsedQuery = q
      }
